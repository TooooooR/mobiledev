...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  // Спільна функція для збереження даних в SharedPreferences
  ...
  void _showEditStationDialog(int index) {
    ...
    // Заповнюємо контролери поточними значеннями
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            children: [
              TextField(controller: _nameController, decoration: const InputDecoration(labelText: 'Назва')),
              TextField(controller: _cpuController, decoration: const InputDecoration(labelText: 'CPU (%)'), keyboardType: TextInputType.number),
              TextField(controller: _ramController, decoration: const InputDecoration(labelText: 'RAM (MB)'), keyboardType: TextInputType.number),
              TextField(controller: _tempController, decoration: const InputDecoration(labelText: 'Температура (°C)'), keyboardType: TextInputType.number),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              setState(() {
                ...
                station.stats = SystemStats(   // ⚠️ тут нюанс (дивись нижче)
          ...
          ElevatedButton(
            onPressed: () async {
              final newStation = Station(
                // Генеруємо унікальний ID для нашого Dropdown
                id: "st_${DateTime.now().millisecondsSinceEpoch}",
                name: _nameController.text.isEmpty ? "PC-${Random().nextInt(100)}" : _nameController.text,
                stats: SystemStats(
                  cpuLoad: double.tryParse(_cpuController.text) ?? 0.0,
                  ramUsage: double.tryParse(_ramController.text) ?? 0.0,
                  temperature: double.tryParse(_tempController.text) ?? 0.0,
              ...
              setState(() {
                currentUser!.stations.add(newStation);
              });
  ...
  void _deleteStation(int index) async {
    setState(() {
      currentUser!.stations.removeAt(index);
    });
    await _saveData();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        // Кнопка назад тепер поверне нас на Home, де спрацює оновлення
        leading: IconButton(
        ...
        child: Column(
          children: [
            ...
            Row(
              ...
              children: [
                const Text('Мої станції:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                IconButton(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text('CPU: ${station.stats.cpuLoad.toStringAsFixed(0)}%'),
                      trailing: IconButton(
